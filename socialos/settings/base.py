...
DATABASES = {
    "default": {
        ...
        "OPTIONS": {
            "connect_timeout": 10,
            # Prevent accidental full-table scans from lingering
...
# axes.backends.AxesStandaloneBackend must be FIRST so it can reject locked
# accounts before Django's backend even tries to authenticate.
# ---------------------------------------------------------------------------
...
SPECTACULAR_SETTINGS = {
    ...
    "DESCRIPTION": (
        ...
        "Supports scheduling, publishing, analytics, inbox, and AI "
        "features across Facebook, Instagram, Twitter/X, and LinkedIn."
    ),
    "VERSION": "1.0.0",
...
# Persisted results via django-celery-results
...
# Fetch one task at a time (fair distribution)
...
LOGGING = {
    ...
    "formatters": {
        "verbose": {
            "format": ("[{asctime}] {levelname} [{name}:{lineno}] " "pid={process:d} tid={thread:d} — {message}"),
            "style": "{",
            "datefmt": "%Y-%m-%d %H:%M:%S",
        },
        "simple": {
